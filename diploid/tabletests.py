...
class GenerationTable(Table):
    ...
    def __init__(self, params, ID, maternal_id, paternal_id, time, sex,
        ...
        if len(set(time)) == 1:
            ...
            raise AttributeError("multiple times have been combined")
...
class PedigreeTable(Table):
    ...
    def get_generation(self, t):
        ...
        mask = self.cols == t
        generation_cols = self.cols[mask]
...
class Trial:
    ...
    def run(self):
        ...
        if self.plot_int:
            self.figs.append(generation_table.plot_subpops())
        while self.t > 0:
            self.pedigree_table.append_generation(generation)
...
class GenotypeArr:
    ...
    def from_generation(cls, generation_table, bin_size=0.01):
        ...
        time = generation_table.time
        return cls(arr, params, time, bin_size)
    ...
    def plot_history(self, log=True):
        ...
        for i in np.arange(9):
            sub.plot(times, np.sum(self.arr[:, :, i], axis=1),
                     color=Constants.subpop_colors[i], linewidth=2)
